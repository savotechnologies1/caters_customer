...
class AuthController extends GetxController implements GetxService {
  ...
  XFile? get pickedGstIn => _pickedGstIn;
  XFile? get pickedPanCard => _pickedPanCard;
  XFile? get pickedCheckImage => _pickedCheckImage;
  ...
  Future<void> getPackageList() async {
      ...
      if (response.statusCode == 200) {
        _packageModel = null as PackageModel;
  ...
  Future<ResponseModel> sendOtp(String phone,) async {
    ...
    ResponseModel? responseModel;
    if (response.statusCode == 200){
      otp = response.body["success"][0]["otp"];
      responseModel = ResponseModel(true, otp.toString());
    }
    return responseModel!;
  }

  Future<ResponseModel> loginWithPhone(String phone , bool status) async {
    Response response = await authRepo!.loginWithNumber(phone);
    ResponseModel responseModel;
    if (response.statusCode == 200){
      if(status){
        authRepo!.saveUserToken(response.body['token']);
  ...
  //   }
  // }
  void pickImage(String type, bool isRemove) async {
    if(isRemove) {
      _pickedLogo = null;
      _pickedCover = null;
      _pickedFssai = null;
      ...
      if (type == "logo") {
        ...
        _pickedFssai = await ImagePicker().pickImage(source: ImageSource.gallery);
      }else if (type == "pan_card"){
        _pickedPanCard = await ImagePicker().pickImage(source: ImageSource.gallery);
      }else if (type == "gstIn"){
        _pickedGstIn = await ImagePicker().pickImage(source: ImageSource.gallery);
      }else if (type == "check"){
        _pickedCheckImage = await ImagePicker().pickImage(source: ImageSource.gallery);
      }
      update();
    }
  }
  ...
  Future<void> registerRestaurant(RestaurantBody restaurantBody) async {
    _isLoading = true;
    update();
    Response response = await authRepo!.registerRestaurant(restaurantBody, _pickedLogo!, _pickedCover!, _pickedFssai! , _pickedPanCard! , _pickedGstIn!);
    if(response.statusCode == 200) {
