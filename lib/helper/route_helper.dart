...
class RouteHelper {
  ...
  static String getPickMapRoute({
    ...
    required bool fromAddAddress,
    required bool canRoute,
    required String route,
  }) {
    return '$pickMap'
        '?fromSignUp=$fromSignUp'
        '&fromAddAddress=$fromAddAddress'
        '&canRoute=$canRoute'
        '&route=$route';
  }
  ...
  static List<GetPage> routes = [
    ...
    GetPage(
      name: pickMap,
      page: () {
        var fromSignUp = Get.parameters['fromSignUp'] == 'true';
        var fromAddAddress = Get.parameters['fromAddAddress'] == 'true';
        var route = Get.parameters['route'] ?? '';
        var canRoute = Get.parameters['canRoute'] == 'true';

        return PickMapScreen(
          fromSignUp: fromSignUp,
          fromAddAddress: fromAddAddress,
          canRoute: canRoute,
          route: route,
    ...
    GetPage(name: orderSuccess, page: () => getRoute(OrderSuccessfulScreen(
      orderID: Get.parameters['id']!, status: Get.parameters['status']!.contains('success') ? 1 : 0, totalAmount: 0.0,
    ))),
    ...
    GetPage(
      ...
      page: () {
        ...
        if (args is CheckoutScreen) {
          // ✅ Safe: arguments is already a CheckoutScreen
          return getRoute(args);
        } else if (fromCart) {
          // ✅ No arguments, but from cart
          return getRoute(
            CheckoutScreen(
              cartList: <CartModel>[], // or pass your cart list here
              fromCart: true,
            ),
          );
        } else {
    ...
    GetPage(name: messages, page: () {
      ...
      notificationBody: _notificationBody, // null allowed ✅
      user: _user,
      index: (Get.parameters['index'] != null && Get.parameters['index'] != 'null')
      ? int.tryParse(Get.parameters['index']!)
          : null,
      conversationID: (Get.parameters['conversation_id'] != null &&
      Get.parameters['conversation_id'] != 'null')
      ? int.tryParse(Get.parameters['conversation_id']!)
          : null,
      ));
  ...
  static Widget getRoute(Widget navigateTo) {
    ...
    if (GetPlatform.isAndroid) {
      ...
      _minimumVersion =
          Get.find<SplashController>().configModel?.appMinimumVersionIos ?? 0;
    }

    if (AppConstants.APP_VERSION < _minimumVersion) {
      return UpdateScreen(isUpdate: true);
    } else if (Get.find<SplashController>().configModel?.maintenanceMode ==
        true) {
