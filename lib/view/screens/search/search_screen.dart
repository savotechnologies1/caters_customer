...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if (_isLoggedIn!) {
      Get.find<pre.SearchController>().getSuggestedFoods();
    }
    Get.find<pre.SearchController>().getHistoryList();
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        if (Get
            .find<pre.SearchController>()
            .isSearchMode) {
          return true;
        } else {
          Get.find<pre.SearchController>().setSearchMode(true);
          return false;
        }
      },
      child: Scaffold(
        ...
        body: SafeArea(child: Padding(
          ...
          child: GetBuilder<pre.SearchController>(builder: (searchController) {
            _searchController.text = searchController.searchText!;
            return Column(children: [

              Center(child: SizedBox(
                  width: Dimensions.WEB_MAX_WIDTH, child: Row(children: [
              ...
              Expanded(
                    ...
                    child: Center(child: SizedBox(
                        ...
                        child: Column(
                            ...
                            children: [
                              ...
                              searchController.historyList!.length > 0 ? Row(
                                  mainAxisAlignment: MainAxisAlignment
                                      .spaceBetween, children: [
                                Text('history'.tr, style: robotoMedium.copyWith(
                                      ...
                                      InkWell(
                                        ...
                                        child: Padding(
                                          ...
                                          child: Icon(Icons.close, color: Theme
                                              .of(context)
                                              .disabledColor, size: 20),
                                        ),
                                      )
                                    ]),
                                    index !=
                                        searchController.historyList!.length - 1
                                        ? Divider()
                                        : SizedBox(),
                                  ]);
                                },
                              ...
                              (_isLoggedIn! &&
                                  searchController.suggestedFoodList != null)
                                  ? Text(
                                'suggestions'.tr, style: robotoMedium.copyWith(
                                  fontSize: Dimensions.fontSizeLarge),
                              )
                                  : SizedBox(),
                              ...
                              (_isLoggedIn! &&
                                  searchController.suggestedFoodList != null)
                                  ? searchController.suggestedFoodList!.length >
                                  0 ? GridView.builder(
                                ...
                                physics: NeverScrollableScrollPhysics(),
                                shrinkWrap: true,
                                itemCount: searchController.suggestedFoodList!
                                    .length,
                                itemBuilder: (context, index) {
                                  return InkWell(
                                    onTap: () {
                                      ResponsiveHelper.isMobile(context) ? Get
                                          .bottomSheet(
                                        ProductBottomSheet(
                                        ...
                                        Dialog(child: ProductBottomSheet(
                                            product: searchController
                                                .suggestedFoodList![index])),
                                      );
                                    },
                  ...
                  ) : SearchResultWidget(searchText: _searchController.text
                      .trim())),
  ...
  void _actionSearch(pre.SearchController searchController, bool isSubmit) {
    if (searchController.isSearchMode || isSubmit) {
      ...
      if (!searchController.isRestaurant!) {
        searchController.allProductList?.forEach((product) {
      ...
      double _maxValue = _prices.isNotEmpty ? _prices.last : 1000;
      Get.dialog(FilterWidget(
        maxValue: _maxValue,
        isRestaurant: searchController.isRestaurant ?? false,
      ));
    }
  }
